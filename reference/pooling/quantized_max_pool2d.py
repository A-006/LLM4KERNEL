...
class Model(nn.Module):
    """
    A model that performs torch.nn.quantized.functional.max_pool2d operation.
    """
    ...
    def forward(self, x):
        """
        Forward pass, applying quantized max pooling 2D.

        Args:
            x (torch.Tensor): Quantized input tensor of shape (N, C, H, W).

        Returns:
            torch.Tensor: Quantized output tensor.
        """
        return Fq.max_pool2d(
            x, kernel_size=self.kernel_size, stride=self.stride,
            padding=self.padding, dilation=self.dilation
        )

def get_inputs():
    # Input shape: (N, C, H, W) = (1, 1, 8, 8)
    input_float = torch.rand(1, 1, 8, 8)
